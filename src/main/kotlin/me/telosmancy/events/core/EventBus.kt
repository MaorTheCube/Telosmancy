...
import me.telosmancy.events.Event
...
object EventBus {
    ...
    fun <T : Event> post(event: T) {
        ...
        val profilerName = profilerNameCache.getOrPut(eventClass) { "telosmancy: ${eventClass.simpleName}" }
...
inline fun <reified P : net.minecraft.network.protocol.Packet<*>> Any.onReceive(
    priority: Int = 0,
    ignoreCancelled: Boolean = false,
    noinline handler: P.(me.telosmancy.events.PacketEvent.Receive) -> Unit
) = EventBus.registerListener(this.javaClass, me.telosmancy.events.PacketEvent.Receive::class.java, priority, ignoreCancelled) {
...
inline fun <reified P : net.minecraft.network.protocol.Packet<*>> Any.onSend(
    priority: Int = 0,
    ignoreCancelled: Boolean = false,
    noinline handler: P.(me.telosmancy.events.PacketEvent.Send) -> Unit
) = EventBus.registerListener(this.javaClass, me.telosmancy.events.PacketEvent.Send::class.java, priority, ignoreCancelled) {
    (it.packet as? P)?.handler(it)
}
