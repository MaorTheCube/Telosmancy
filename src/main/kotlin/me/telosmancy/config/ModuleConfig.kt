...
class ModuleConfig internal constructor(file: File) {
    ...
    /**
     * Main constructor for Addons. (config/telosmancy/addons/{fileName})
     */
    constructor(fileName: String) : this(File(Telosmancy.configFile, "addons/$fileName"))
    ...
    private val file: File = file.apply {
        try {
            ...
            Telosmancy.logger.error("Error initializing module config", e)
        }
    }
    ...
    fun save() {
        try {
            ...
            Telosmancy.logger.error("Error saving module config.", e)
        }
    }
